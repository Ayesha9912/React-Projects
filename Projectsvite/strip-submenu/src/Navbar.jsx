...
export default function Navbar(){
    ...
    const displaySubmenu =(e)=>{
      const page = e.target.textContent;   // developers , products, 
      const tempBtn = e.target.getBoundingClientRect();  // top, left, bottom, right, x, y
  ...
  return(
    <>
    <section className='w-full  p-4 bg-slate-500 flex justify-between '>
        <div className='w-[90%] m-auto flex justify-between items-center'>
          ...
          <button className='text-white hover:bg-slate-500 transform duration-300 hidden md:flex bg-black p-2 rounded-md cursor-pointer'><a href="">Sign Up</a></button>
            <div onClick={openSidebar} className='text-[25px] text-white flex md:hidden  '>
            <FaBars/>
            </div>
        </div>
    </section>
